	  ...
	  if(authData){
		...
		$scope.listItemSubmit = function () {
			...
			if (category !== null && task !== null) {
				$http.get('../scripts/php/mongoMileList.php?firstName='+$scope.userName+'&lastName='+$scope.lastName+'&provider='+$scope.provider+'&category='+category+'&task='+task+'&title='+title)
			        .success(function(mongoData){
						$http.get('../scripts/php/getMileList.php?firstName='+$scope.userName+'&lastName='+$scope.lastName+'&provider='+$scope.provider)
				        .success(function(mongoData){
				        	$scope.list = mongoData.list;
				        	$route.reload();
					    })
